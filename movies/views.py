...
def recommend_movies(request):
    ...
    if not watched_genres:
        favorite_genre = "Aucun genre trouvé"
        recommended_movies = []
    else:
        ...
        # Obtenir les films recommandés du genre préféré
        recommended_movies = Movie.objects.filter(genre=favorite_genre).exclude(watchedmovie__user=user)

    if recommended_movies:
        recommendation_index = request.session.get('recommendation_index', 0)
        recommendation_index = recommendation_index % len(recommended_movies)
        recommended_movie = recommended_movies[recommendation_index]
    else:
    ...
    if request.headers.get('x-requested-with') == 'XMLHttpRequest':
        movie_data = {
            ...
            'duration': recommended_movie.duration,
            'genre': recommended_movie.genre,
            'image': recommended_movie.image,
        }
